 ...
 * 
 * v2: UnifiedRuleAdapter対応
 */

import { v4 as uuidv4 } from 'uuid';
import { UnifiedRuleAdapter, ConstraintGroup } from './UnifiedRuleAdapter';
...
export async function getAllConstraintGroups(): Promise<ConstraintGroup[]> {
  try {
    return await UnifiedRuleAdapter.getAllConstraintGroupsWithFallback();
  } catch (error) {
...
export async function getConstraintsByGroupId(groupId: string): Promise<EnhancedConstraint[]> {
  try {
    return await UnifiedRuleAdapter.getConstraintsByGroupIdWithFallback(groupId);
  } catch (error) {
