...
async function generateDec10To16Shifts() {
  console.log('🚀 Generating shifts for December 10-16, 2025...\n');
  ...
  if (multiDayShifts.length > 0) {
    ...
    if (verifyError) {
      ...
      if (verifyData.length > 0) {
        ...
        // Show all sets
        sets.forEach(setId => {
          const setShifts = verifyData.filter(s => s.multi_day_set_id === setId);
          console.log(`\n  ${setId}:`);
          setShifts.forEach(s => {
  ...
  console.log('\n✅ Complete! Refresh the UI to see the changes.');
}

generateDec10To16Shifts().catch(console.error);
