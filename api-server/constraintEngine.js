...
const constraintManager_1 = require("./constraintManager");
class ConstraintEngine {
     ...
     * 指定拠点の制約条件を読み込み
     */
    loadConstraints(location) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log('🔄 [CONSTRAINT] Loading constraints for location:', location);
            try {
                if (location) {
                    this.constraints = yield constraintManager_1.ConstraintManager.getActiveConstraintsByLocation(location);
                }
                else {
                    const allConstraints = yield constraintManager_1.ConstraintManager.getAllConstraints();
     ...
     */
    validateShiftAssignment(employee, proposedShift, existingShifts) {
        return __awaiter(this, void 0, void 0, function* () {
            const violations = [];
            let canProceed = true;
     ...
     * 個別制約のチェック
     */
    checkConstraint(constraint, employee, proposedShift, existingShifts) {
     ...
     * 最大連続出勤日数チェック
     */
    checkMaxConsecutiveDays(constraint, employee, proposedShift, existingShifts) {
     ...
     * 最小休息時間チェック
     */
    checkMinRestHours(constraint, employee, proposedShift, existingShifts) {
        const employeeShifts = existingShifts.filter(s => s.employee_id === employee.id);
     ...
     * 週間最大労働時間チェック
     */
    checkMaxWeeklyHours(constraint, employee, proposedShift, existingShifts) {
        const proposedDate = new Date(proposedShift.shift_date);
        const weekStart = new Date(proposedDate);
        weekStart.setDate(proposedDate.getDate() - proposedDate.getDay());
        const weekEnd = new Date(weekStart);
        weekEnd.setDate(weekStart.getDate() + 6);
        const weekShifts = existingShifts.filter(s => s.employee_id === employee.id &&
     ...
     * 月間最大労働時間チェック
     */
    checkMaxMonthlyHours(constraint, employee, proposedShift, existingShifts) {
        const proposedDate = new Date(proposedShift.shift_date);
        ...
        const monthShifts = existingShifts.filter(s => s.employee_id === employee.id &&
     ...
     * 1日最大シフト数チェック
     */
    checkMaxShiftsPerDay(constraint, employee, proposedShift, existingShifts) {
        const sameDayShifts = existingShifts.filter(s => s.employee_id === employee.id && s.shift_date === proposedShift.shift_date);
     ...
     * 制約違反をログに記録
     */
    logViolations(violations, batchId) {
        return __awaiter(this, void 0, void 0, function* () {
            if (violations.length === 0)
                return;
            try {
                yield constraintManager_1.ConstraintManager.logViolations(violations, batchId);
            }
            catch (error) {
