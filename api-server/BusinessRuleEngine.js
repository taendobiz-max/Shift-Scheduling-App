...
exports.getRuleEngine = getRuleEngine;
exports.resetRuleEngine = resetRuleEngine;
...
class BusinessRuleEngine {
     ...
     * ペア業務を処理
     */
    async handlePairBusiness(context) {
        const applicableRules = this.getApplicableRules(context);
        for (const rule of applicableRules) {
    ...
    async checkConstraints(context) {
        const results = [];
        const applicableRules = this.getApplicableRules(context).filter(rule => rule.rule_type === 'constraint_check');
        for (const rule of applicableRules) {
            // 制約チェックロジックは今後実装
            // 現時点ではプレースホルダー
        }
     ...
     */
    async teamRotationFilterHandler(context, action) {
        try {
     ...
     * 班指定フィルタハンドラー
     */
    async teamFilterHandler(context, action) {
        try {
     ...
     * 連続ペア割り当てハンドラー
     */
    async consecutivePairHandler(context, action) {
        try {
     ...
     */
    async rollCallFilterHandler(context, action) {
        try {
     ...
     * スキルフィルタハンドラー
     */
    async skillFilterHandler(context, action) {
        try {
            const { business, availableEmployees } = context;
 ...
 */
function getRuleEngine(supabaseUrl, supabaseKey) {
    if (!engineInstance) {
