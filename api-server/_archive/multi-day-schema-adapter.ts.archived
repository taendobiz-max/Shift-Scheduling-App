 ...
 */
export function adaptBusinessMasterForMultiDay(dbBusiness: any): BusinessMaster {
  const business = { ...dbBusiness } as BusinessMaster;
  ...
  let applicableDates: 'even_days' | 'odd_days' | 'all' = 'all';
  if (business.方向 === 'even') {
  ...
  // ローテーションルールを構築
  const rotationRule: RotationRule | undefined = (applicableDates !== 'all' || teamFilter) ? {
  ...
  const multiDayConfig: MultiDayBusinessConfig = {
 ...
 */
export function adaptBusinessMastersForMultiDay(dbBusinesses: any[]): BusinessMaster[] {
