...
**作成日**: 2026年2月28日  
**最終更新日**: 2026年8月21日（P0セキュリティ強化・R1原子保存・生成ルール警告対応）
...
本システムは、フロントエンド（React SPA）とバックエンド（Supabase + 認証済みAPIサーバー）で構成されます。画面からの更新操作はJWTを付与したAPIを経由し、APIサーバーがバックエンド専用キーでSupabaseに接続します。
...
Supabase Auth を利用したメールアドレスとパスワードによる認証を実装しています。認証成功後はJWTを取得し、画面のAPI呼出しに `Authorization: Bearer <JWT>` を付与します。APIサーバーはJWTをSupabase Authで検証し、`users.role` に基づき、一般利用者（1）・管理者（2）・スーパー管理者（3）のRBACを適用します。

- 認証済みユーザーだけがRLS保護されたデータを参照できます。
- 管理操作は管理者以上、ユーザー管理はスーパー管理者だけに制限されます。
- APIの更新操作は `audit_logs` に操作主体・対象・経路・要約を記録します。
- セッション情報はSupabaseクライアントのストレージに保持されます。
...
6. マトリクス形式で結果表示
7. インタラクティブ編集（担当済みセル間のドラッグ入替、セルクリックでの変更・削除・候補選択）
8. 「シフトを保存」で `/api/shifts/bulk` に保存
9. APIが対象期間・拠点の既存シフト削除と新規シフト挿入を単一トランザクションで実行
```
...
- 生成時点では既存シフトを削除しません。生成・編集・保存のいずれかが失敗しても、確定済みシフトを失わないためです。
- 保存処理は `/api/shifts/bulk` を呼び出します。PostgreSQL関数 `replace_shifts_atomically` が、対象期間・営業所の既存シフト削除と新規シフト挿入を**同一トランザクション**で実行します。
- `multi_day_set_id` / `multi_day_info` も保持して保存します。
...
| 部分再生成 | 対象日・選択業務または未割当業務だけを再生成し、成功時に当日分を原子的に置換 |
...
| 機能 | 説明 |
|---|---|
| 休暇登録 | 従業員、日付、休暇種別、理由を選択して登録。期間登録は1回の一括APIで原子的に登録 |
...
`(employee_id, vacation_date)` は一意です。`POST /api/vacations/bulk` は重複を含む期間登録を409で拒否し、バッチ内の一部だけが登録される状態を防ぎます。更新・削除も認証済みAPIから実行され、監査ログに記録されます。
