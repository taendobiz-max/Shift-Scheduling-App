...
function checkDailyWorkHours(employeeId: string, newBusiness: any, currentShifts: Shift[], targetDate?: string): { canAssign: boolean; totalHours: number; message?: string } {
  ...
  console.log(`📊 [DAILY_HOURS_CHECK] Employee: ${employeeId}, Business: ${businessName}, TargetDate: ${targetDate}`);
  console.log(`📊 [DAILY_HOURS_CHECK] Existing shifts count: ${employeeShifts.length}`);
  
  // 既存シフトの労働時間を計算
  let totalHours = 0;
  for (const shift of employeeShifts) {
    const shiftHours = calculateShiftHours(shift.start_time, shift.end_time);
    console.log(`📊 [DAILY_HOURS_CHECK] Existing shift: ${shift.business_group} (${shift.start_time}-${shift.end_time}) = ${shiftHours.toFixed(1)}h`);
  ...
  console.log(`📊 [DAILY_HOURS_CHECK] New business: ${businessName} (${newStart}-${newEnd}) = ${newHours.toFixed(1)}h`);
  console.log(`📊 [DAILY_HOURS_CHECK] Total hours: ${totalHours.toFixed(1)} + ${newHours.toFixed(1)} = ${(totalHours + newHours).toFixed(1)}h (limit: ${MAX_DAILY_WORK_HOURS}h)`);
  totalHours += newHours;
